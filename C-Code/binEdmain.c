...
int main(){
	...
	convert_ctoi(&work,base);
	convert_gfe2gfe1x(&baseP[0], &work);
	convert_ctoi(&work,base+32);
	convert_gfe2gfe1x(&baseP[1], &work);
	convert_ctoi(&work,_2base);
	convert_gfe2gfe1x(&baseP[2], &work);
	convert_ctoi(&work,_2base+32);
	convert_gfe2gfe1x(&baseP[3], &work);

	MEASURE({
		scalar_mult_fixed_base(npp, n, baseP);
	});
